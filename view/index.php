...
$publicacoes_listagem = $controller->getPublicacoes();

session_start();

// echo "<pre>";
// var_dump($publicacoes_listagem);die;
// echo "</pre>";
...
<div class="container" x-data="{ rightSide: false, leftSide: false }">
      ...
      <div class="timeline">
        ...
        <div class="timeline-right">
          <div class="status box">
            ...
            <form action="../controller/Controller.php" id="formComentar" method="post">
              <input type="hidden" name="method" value="add">
              <input type="hidden" name="user_id" value="<?= $_SESSION['user_id'] ?? null ?>">

              <div class="status-main">
                <img src="https://cdn-icons-png.flaticon.com/512/149/149071.png" class="status-img">
                <textarea class="status-textarea" name="texto" placeholder="Digite algo..."></textarea>
              </div>
              <div class="status-actions">
                ...
                <button type="button" class="status-share" id="publicarComentario">Comentar</button>
              </form>
            </div>
          </div>
          <!-- Inicio dos post -->
          <?php if($publicacoes_listagem != 0) : ?>
            <?php foreach ($publicacoes_listagem as $key => $publicacao) : ?>
              <div class="album box">
                <div class="status-main">
                  <img src="https://cdn-icons-png.flaticon.com/512/149/149071.png" class="status-img" />
                  <div class="album-detail">
                    <div class="album-title"><strong><?= ucfirst($publicacao['u_login']) ?></strong> criou um comentário</div>
                    <div class="album-date"><?= date('d/m/Y H:i', strtotime($publicacao['p_created'])) ?></div>
                  </div>
                  <button class="intro-menu"></button>
                </div>
                ...
                <div class="album-actions">
                  <a href="#" class="album-action" onclick="curtidaClick(<?= $publicacao['p_id'] ?>)">
                    <svg stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                      <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
                    </svg>
                    <!-- numero de curtida -->
                    <span id="numeroCurtida<?= $publicacao['p_id'] ?>"><?= $publicacao['total_curtidas'] ?? 0 ?></span>
                  </a>
                  <a href="#" class="album-action" onclick="openModal(<?= $publicacao['p_id'] ?>)">
                    ...
                    <span id="numeroComentario<?= $publicacao['p_id'] ?>"><?= $publicacao['total'] ?></span>
                  </a>
                </div>            
              </div>
            <?php endforeach; ?>            
          <?php else: ?>
            <div class="album box">
              <div class="status-main">
                <h3>Nenhum publicação encontrada</h3>
              </div>
            </div>
            <?php endif; ?>
          <!-- Fim dos post -->
  ...
  <div class="right-side" :class="{ 'active': rightSide }">
    <div class="account">
      ...
      <?php if(isset($_SESSION['login'])) : ?>
        <span class="account-user" id="btnSair" style="font-size:19px;color:white;cursor:pointer">
          Sair
          <span style="margin-left:13px"> ▼</span>
        </span>
        <form action="../controller/Controller.php" id='formLogoff' method="post">
          <input type="hidden" name="method" value="logoff">
        </form>
      <?php else: ?>
        <span class="account-user" data-bs-toggle="modal" data-bs-target="#modalEntrar" style="font-size:19px;color:white;cursor:pointer">
          Entrar
          <span style="margin-left:13px"> ▼</span>
        </span>
      <?php endif ?>
  ...
  <!-- Modal -->
  <div class="modal fade" id="modalComentario" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form action="../controller/Controller.php" method="post">
          <div class="modal-header">
            <h5 class="modal-title" style="color:black" id="exampleModalLabel">Adicionar comentario</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body" id="bodyModalComentario">
            <!-- <div class="col-md-12" style="color:black">
              <span><b><i style="margin-right:10px">Guittex 10:45 04/12:</i></b>sad asd</span>
            </div>             -->
          </div>
          <div class="modal-body">
            <input type="hidden" name="publicacao_id" id="idPublicacaoModal">
            <input type="hidden" name="user_id" value="<?=  $_SESSION['user_id'] ?>">
            <input type="hidden" name="method" value="addComentario">
            <textarea name="comentarioModal" class='form-control' id="comentarioModal" placeholder="Comente aqui..."></textarea>
          </div>
  ...
  <div class="modal fade" id="modalCadastrar" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form action="../controller/Controller.php" method="post">
          <div class="modal-header">
            <h5 class="modal-title" style="color:black" id="exampleModalLabel">Cadastrar Usuario</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>       
          <div class="modal-body">
            ...
            <div class="col-md-12" style="margin-bottom:10px">
              ...
              <input type="text" class="form-control" placeholder="Digite..." name="login" id="loginEntrarModal">
            </div>
            <div class="col-md-12">
              <label for="" style="color:black">Senha</label>
              <input type="password" class="form-control" placeholder="Digite..." name="senha" id="SenhaEntrarModal">
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Fechar</button>
  ...
  <div class="modal fade" id="modalEntrar" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form action="../controller/Controller.php" method="POST">
          <div class="modal-header">
            <h5 class="modal-title" style="color:black" id="exampleModalLabel">Entrar</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>        
          <div class="modal-body">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="openModalCadastrar">Cadastrar Usuario</button>
            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Fechar</button>
...
<!-- partial -->
  ...
  <script>
    function curtidaClick(publicacao_id)
    {
      user_id = <?= $_SESSION['user_id'] ?? 'false'; ?>

      if(user_id == false){
        alert("É necessário entrar antes de curtir essa publicação");
        ...
        $.ajax({
            ...
            success: function(retorno) {
              if (retorno != 'false') {
                var valor_antigo = parseInt($(`#numeroCurtida${publicacao_id}`).html());

                var valor_atual = valor_antigo + 1;

                $(`#numeroCurtida${publicacao_id}`).html(valor_atual);
              }else{
                var curtida_total = parseInt($(`#numeroCurtida${publicacao_id}`).html());

                console.log(curtida_total);

                if(parseInt($(`#numeroCurtida${publicacao_id}`).html()) >= 1){
                  var valor_antigo = parseInt($(`#numeroCurtida${publicacao_id}`).html());
    ...
    function openModal(publicacao_id)
    {
      $("#idPublicacaoModal").val("");

      $("#idPublicacaoModal").val(publicacao_id);
      ...
      $.ajax({
          ...
          data: {
              publicacao_id: publicacao_id,
              method: 'getPublicacaoAjax'
          },
          beforeSend: function(xhr) {
              xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"').val());
          },
          success: function(retorno) {
            $("#bodyModalComentario").empty();

            retorno = JSON.parse(retorno);

            if (retorno != false) {
                $(retorno).each(function(){
                  var html = '<div class="col-md-12" style="color:black">';
                    html +=   `<span><b><i style="margin-right:10px">${this.login} ${this.data_cadastro}:</i></b>${this.comentario}</span>`;
                    html += '</div><hr>';   
                    
                    $("#bodyModalComentario").append(html);
                });
            }else{
              $("#bodyModalComentario").append("<span>Nenhum comentario</span>");
    ...
    $("#openModalCadastrar").click(function(){
    ...
    $("#publicarComentario").on("click", function(){
      user_id = <?= $_SESSION['user_id'] ?? 'false'; ?>

      if(user_id == false){
        alert("É necessário entrar antes de prosseguir com o comentario");
